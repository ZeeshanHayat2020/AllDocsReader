...
public class DocumentPropertiesLoader extends AsyncTaskLoader<List<CharSequence>> {
    ...
    public List<CharSequence> loadInBackground() {
        ...
        if (mCursor != null) {
            ...
            if (indexSize >= 0) {
                ...
                properties.add(getProperty(null, names[1], UtilsPdfViewer.parseFileSize(fileSize)));
            }
    ...
    private CharSequence getProperty(final JSONObject json, String name, String specName) {
        ...
        if (specName.endsWith("Date")) {
            final Context context = getContext();
            try {
                property.append(value.equals("-") ? value : UtilsPdfViewer.parseDate(value));
            } catch (ParseException e) {
                Log.d(TAG, e.getMessage() + " for " + value + " at offset: " + e.getErrorOffset());
